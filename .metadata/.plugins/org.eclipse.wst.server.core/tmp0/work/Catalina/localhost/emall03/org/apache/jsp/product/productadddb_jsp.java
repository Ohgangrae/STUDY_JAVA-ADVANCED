 ...
 * Generated at: 2022-04-23 08:53:50 UTC
...
while(iter.hasNext()) {  // 요소가 있으면 계속 반복 없으면 종료
    ...
    if(item.isFormField()) { // 이름과 값으로 즉 맵 형태 쌍으로 구성이 되어 있는지 확인
        String value = item.getString("utf-8"); // 한글 처리 중요!!!
       	if (pname.equals("name")) name = value;
        else if (pname.equals("price")) price = value;
    }
    else {
        if (pname.equals("image")) {
            image = item.getName();
            file = item.get();
            //서버에 사진 저장
...
if (dao.insert(name, price, image)) {
